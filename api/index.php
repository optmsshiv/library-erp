...
switch ($action) {
    ...
    // ══════════════════════════════════
    case 'get_staff':
        $rows = $db->query("SELECT id,name,role,email,phone,username,perm_students,perm_fees,perm_books,perm_expenses,perm_reports,perm_staff,perm_settings,COALESCE(perm_whatsapp,1) AS perm_whatsapp,COALESCE(perm_notifications,1) AS perm_notifications,act_perms,status FROM staff ORDER BY created_at")->fetchAll();
        foreach ($rows as &$r) {
            if (isset($r['act_perms']) && is_string($r['act_perms'])) {
                $r['act_perms'] = json_decode($r['act_perms'], true) ?? [];
            }
        }
        unset($r);
        jsonResponse($rows);
    ...
    case 'save_staff':
        ...
        if ($isEdit) {
            ...
            $actPermsJson = json_encode($d['actPerms'] ?? []);
            if (!empty($d['password'])) {
                $newHash = password_hash($d['password'], PASSWORD_BCRYPT);
                $db->prepare("UPDATE staff SET name=?,role=?,email=?,phone=?,username=?,password_hash=?,perm_students=?,perm_fees=?,perm_books=?,perm_expenses=?,perm_reports=?,perm_staff=?,perm_settings=?,perm_whatsapp=?,perm_notifications=?,act_perms=? WHERE id=?")
                    ->execute([$d['name'],$d['role'],$d['email'],$d['phone'] ?? '',$d['username'] ?? '',
                        ...
                        (int)($perms['whatsapp'] ?? 1),(int)($perms['notifications'] ?? 1),
                        $actPermsJson, $d['id']]);
            } else {
                $db->prepare("UPDATE staff SET name=?,role=?,email=?,phone=?,username=?,perm_students=?,perm_fees=?,perm_books=?,perm_expenses=?,perm_reports=?,perm_staff=?,perm_settings=?,perm_whatsapp=?,perm_notifications=?,act_perms=? WHERE id=?")
                    ->execute([$d['name'],$d['role'],$d['email'],$d['phone'] ?? '',$d['username'] ?? '',
                        (int)($perms['students'] ?? 0),(int)($perms['fees'] ?? 0),(int)($perms['books'] ?? 0),
                        (int)($perms['expenses'] ?? 0),(int)($perms['reports'] ?? 0),(int)($perms['staff'] ?? 0),(int)($perms['settings'] ?? 0),
                        (int)($perms['whatsapp'] ?? 1),(int)($perms['notifications'] ?? 1),
            ...
            $actPermsJson = json_encode($d['actPerms'] ?? []);
            $db->prepare("INSERT INTO staff (id,name,role,email,phone,username,password_hash,perm_students,perm_fees,perm_books,perm_expenses,perm_reports,perm_staff,perm_settings,perm_whatsapp,perm_notifications,act_perms,status) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")
    ...
    // ══════════════════════════════════════════════════════════════
    // BIOMETRIC DEVICE INTEGRATION — eSSL X990 + Realtime RS9N
    // ZK/eSSL iClock PUSH Protocol (ADMS)
    // ══════════════════════════════════════════════════════════════

    // ── Step 1: Device calls GET /iclock/cdata?SN=XXXXX&options=all ──
    // We respond with server config so device knows what to do next
    case 'iclock_cdata':
    case 'biometric_init': {
        ...
        $db->prepare("INSERT INTO biometric_devices (serial_number, ip_address, last_seen, status)
            ...
            ->execute([$sn, $ip, $ip]);
        // Respond in iClock protocol format
...
";
    ...
    // POST /api/index.php?action=biometric_push
    // Body: ATTLOG	UserID	Timestamp	Status	Verify	WorkCode
...
    // OR JSON push from newer firmware / Realtime RS9N
    case 'biometric_push': {
        ...
        // ── Parse format ──
        // Try JSON first (Realtime RS9N / newer eSSL)
        $jsonData = json_decode($rawBody, true);

        if ($jsonData && isset($jsonData['RealTime']['PunchLog'])) {
            // Modern JSON push format
            $pl     = $jsonData['RealTime']['PunchLog'];
            $userId = $pl['UserId'] ?? '';
            $logTime= $pl['LogTime'] ?? date('Y-m-d H:i:s');
            $type   = strtolower($pl['Type'] ?? 'checkin');
            $verify = strtolower($pl['InputType'] ?? 'fingerprint');
            if ($userId) {
                $rows = [[$userId, $logTime, ($type === 'checkout' ? 1 : 0), $verify]];
            } else { $rows = []; }
        } elseif (strpos($rawBody, 'ATTLOG') !== false || strpos($rawBody, "	") !== false) {
            // Standard iClock ATTLOG text format:
...
", $rawBody) as $line) {
                ...
                if (count($parts) >= 2) {
                    $rows[] = [
                        $parts[0],                     // UserID
                        $parts[1],                     // Timestamp
                        (int)($parts[2] ?? 0),         // Status (0=check_in, 1=check_out)
                        $parts[3] ?? 'fingerprint'     // VerifyType
                    ];
                }
            }
        } elseif ($jsonData && is_array($jsonData)) {
            // Array of punch records
            $rows = [];
            $list = isset($jsonData[0]) ? $jsonData : [$jsonData];
            foreach ($list as $rec) {
                $userId = $rec['user_id'] ?? $rec['UserId'] ?? $rec['userid'] ?? '';
                $time   = $rec['punch_time'] ?? $rec['LogTime'] ?? $rec['time'] ?? date('Y-m-d H:i:s');
                $status = $rec['status'] ?? $rec['Status'] ?? 0;
                $verify = $rec['verify'] ?? $rec['InputType'] ?? 'fingerprint';
                if ($userId) $rows[] = [$userId, $time, (int)$status, $verify];
            }
        } else {
            // Fallback: try key=value or query string body
            parse_str($rawBody, $params);
            $userId = $params['user_id'] ?? $params['UserId'] ?? $_GET['user_id'] ?? '';
            $time   = $params['punch_time'] ?? $params['time'] ?? date('Y-m-d H:i:s');
            $status = (int)($params['status'] ?? 0);
            $verify = $params['verify'] ?? 'fingerprint';
            $rows   = $userId ? [[$userId, $time, $status, $verify]] : [];
        }
        ...
        foreach ($rows as [$userId, $rawTime, $statusCode, $verifyType]) {
            ...
            $punchTs  = strtotime(str_replace('T', ' ', $rawTime));
            if (!$punchTs) $punchTs = time();
            $punchDt  = date('Y-m-d H:i:s', $punchTs);
            $punchDate= date('Y-m-d', $punchTs);
            $punchTime= date('H:i:s', $punchTs);

            // Determine check_in / check_out from status code
            // 0 = check_in, 1 = check_out, 4 = break_out etc.
            ...
            // Map device user_id to student
            // Convention: device user_id = student id (e.g. STU-001) OR numeric enrollment number
            $stuStmt = $db->prepare(
                "SELECT s.*, b.name as batch_name, b.start_time as batch_start
                 FROM students s
                 LEFT JOIN batches b ON b.id = s.batch_id
                 WHERE s.id = ? OR s.enrollment_no = ? LIMIT 1"
            );
            $stuStmt->execute([$userId, $userId]);
            $stu = $stuStmt->fetch(PDO::FETCH_ASSOC);

            if (!$stu) {
                // Try matching by numeric part (device may store just number)
                $stuStmt2 = $db->prepare(
                    "SELECT s.*, b.name as batch_name, b.start_time as batch_start
                     FROM students s
                     LEFT JOIN batches b ON b.id = s.batch_id
                     WHERE s.id LIKE ? OR s.id = ? LIMIT 1"
                );
                $stuStmt2->execute(['%'.$userId, 'STU-'.ltrim($userId,'0')]);
                $stu = $stuStmt2->fetch(PDO::FETCH_ASSOC);
            }

            if (!$stu) { $denied++; continue; }

            // Update biometric_punches with student_id
            ...
            // Check fee gate — deny entry if overdue (optional, controlled by setting)
            $settRow = $db->query("SELECT * FROM settings WHERE id=1")->fetch();
            $feeGate = $settRow['biometric_fee_gate'] ?? 0;
            if ($feeGate && $stu['fee_status'] === 'overdue') {
                $denied++;
                // Log denial
                addActivity($db, '🚫', 'rgba(239,68,68,.14)',
                    "<strong>{$stu['fname']} {$stu['lname']}</strong> entry DENIED — fee overdue");
                continue;
            }
            ...
            $existing->execute([$stu['id'], $punchDate]);
            $att = $existing->fetch(PDO::FETCH_ASSOC);

            if (!$att) {
                // First punch of day = check_in
                $isLate = 0; $lateMin = 0;
                if (!empty($stu['batch_start'])) {
                    $batchTs = strtotime($punchDate . ' ' . $stu['batch_start']);
                    $grace   = 15 * 60;
                    if ($punchTs > $batchTs + $grace) {
                        $isLate = 1;
                        $lateMin = (int)round(($punchTs - $batchTs) / 60);
                    }
                }
                $db->prepare("INSERT INTO student_attendance
                    ...
                    ON DUPLICATE KEY UPDATE check_in=VALUES(check_in), is_late=VALUES(is_late),
                        late_minutes=VALUES(late_minutes), marked_by='biometric'")
                    ->execute([$stu['id'], $punchDate, 'present', $punchTime,
                        $isLate, $lateMin, 'biometric_device', $sn]);
                ...
                $lateMsg = $isLate ? " ⚠ Late by {$lateMin}min" : '';
                addActivity($db, '🖐️', 'rgba(22,163,74,.14)',
                    "<strong>{$stu['fname']} {$stu['lname']}</strong> checked in via biometric at $punchTime$lateMsg");

            } elseif (!$att['check_out'] && $punchType === 'check_out') {
                // Second punch = check_out
                $db->prepare("UPDATE student_attendance SET check_out=?, marked_by='biometric_device'
                    WHERE student_id=? AND date=?")
                    ->execute([$punchTime, $stu['id'], $punchDate]);

                addActivity($db, '🖐️', 'rgba(100,116,139,.14)',
                    "<strong>{$stu['fname']} {$stu['lname']}</strong> checked out at $punchTime");
            }

            // Update device punch count
            $db->prepare("UPDATE biometric_devices SET total_punches=total_punches+1, last_seen=NOW()
                WHERE serial_number=?")
                ->execute([$sn]);
...
";
    ...
    // ── Heartbeat / keepalive from device ──
    case 'biometric_heartbeat': {
        $sn = $_GET['SN'] ?? $_GET['sn'] ?? 'UNKNOWN';
        $ip = $_SERVER['HTTP_X_FORWARDED_FOR'] ?? $_SERVER['REMOTE_ADDR'] ?? '';
        try {
            $db->prepare("UPDATE biometric_devices SET last_seen=NOW(), status='online', ip_address=?
                WHERE serial_number=?")
                ->execute([$ip, $sn]);
        } catch(Exception $e) {}
        header('Content-Type: text/plain');
...
";
    ...
    // ── Get biometric devices list for admin dashboard ──
    case 'get_biometric_devices': {
        try {
            ...
            // Mark devices offline if not seen in 5 min
            $db->exec("UPDATE biometric_devices SET status='offline'
                WHERE last_seen < DATE_SUB(NOW(), INTERVAL 5 MINUTE)");
            $devices = $db->query("SELECT * FROM biometric_devices ORDER BY last_seen DESC")
                ->fetchAll(PDO::FETCH_ASSOC);
    ...
    case 'get_biometric_punches': {
        ...
        try {
            ...
            $punches->execute([$date]);
            $rows = $punches->fetchAll(PDO::FETCH_ASSOC);
        } catch(Exception $e) { $rows = []; }
        jsonResponse(['punches' => $rows]);
        break;
    }

    // ── Toggle fee gate (block overdue students at door) ──
    case 'set_fee_gate': {
        if ($method !== 'POST') jsonError('Method not allowed', 405);
        $d = getInput();
        ...
        try {
            ...
            $db->prepare("UPDATE settings SET biometric_fee_gate=? WHERE id=1")->execute([$enabled]);
        } catch(Exception $e) {}
        jsonResponse(['ok' => true, 'fee_gate' => $enabled]);
        break;
    }        break;
